...
*   **语义检索 (Semantic Search)**：集成硅基流动 `BAAI/bge-m3` **免费**模型，支持基于向量相似度的深度记忆召回，不再局限于关键词匹配。
*   **主动遗忘 (LRU & Compaction)**：内置 `memory_lru.py` 和 `compact.py`（默认使用 **Qwen2.5-7B 免费模型**），自动清理低价值记忆，保持系统轻量化。
...
本系统默认使用硅基流动的**免费**资源即可运行。你需要设置 API Key：
...
> **提示**：系统默认已配置好免费模型参数，无需额外的模型环境配置。
> *   Embedding 模型：`BAAI/bge-m3`
> *   总结/检索模型：`Qwen/Qwen2.5-7B-Instruct`
...
# 系统会自动调用 SiliconFlow API 进行语义匹配
...
本项目的记忆管理设计灵感部分来源于 **FreeTodo** —— 一个探索 AI 任务管理与记忆持久化的早期实验项目。FreeTodo 在架构上验证了"记忆即状态"的核心理念，其对记忆生命周期管理的思考为 X-Memory 的设计奠定了基础。
