...
# Note: This model is used for semantic reranking/candidate selection, not vector embedding.
# For vector embedding, ensure memory_store.py uses a dedicated embedding endpoint (e.g., bge-m3).
LLM_MODEL = os.environ.get("SMART_RECALL_MODEL", "Qwen/Qwen2.5-7B-Instruct")
...
def _semantic_filter(query: str, headers: list[dict], limit: int = 50) -> list[dict]:
    ...
    try:
        ...
        for row in rows:
            ...
            target_header = None
            for h in headers:
                # Simple matching: check if id is in filename or description
                if str(row["source_id"]) in h.get("description", "") or str(row["source_id"]) in h.get("filename", ""):
                    target_header = h
                    break
            
            if target_header:
                vec = struct.unpack(f'{len(row["embedding"])//4}f', row["embedding"])
                score = _cosine_sim(q_vec, list(vec))
                scored.append((score, target_header))
